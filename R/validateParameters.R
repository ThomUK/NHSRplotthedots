...
validateParameters <- function(df, valueField, dateField, facetField, options) {
  ...
  # if provided, options$rebase must be:
  # EITHER: of character type, length 1, and a column name from within the data frame
  ...
  if (!is.null(options$rebase)) {
    if (all(isDate(options$rebase))) {
      ...
      if (length(options$rebase) > 1) {
        stop("spc: options$rebase argument must be either:
             1) a vector of length 1 specifying a column name from within the dataset,
             or 2) a vector of dates in '%Y-%m-%d' format.")
      }
